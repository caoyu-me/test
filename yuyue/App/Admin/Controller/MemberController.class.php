...
class MemberController extends ComController
{
    public function index()
    {
        ...
        $count = $user->field("{$prefix}admin.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title")
            ->order($order)
            ->join("{$prefix}auth_group_access ON {$prefix}admin.id = {$prefix}auth_group_access.uid")
    ...
    public function edit()
    {
        ...
        if ($uid) {
            ...
            $prefix = C('DB_PREFIX');
            $user = M('admin');
            $member = $user->field("{$prefix}admin.*,{$prefix}auth_group_access.group_id")->join("{$prefix}auth_group_access ON {$prefix}admin.id = {$prefix}auth_group_access.uid")->where("{$prefix}admin.id=$uid")->find();
    ...
    public function update($ajax = '')
    {
        ...
        if (!$uid) {
            ...
            M('admin')->data($data)->where("id=$uid")->save();
