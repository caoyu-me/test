...
//            die;
                if(time()- $timer > $this->timer){
                    ...
                    //小于设置时间
                    //1:在指定时间内不能重复发送
                    return $arr=array('err'=>1,'invalid'=>$this->timer,'sytimer'=>60-(time()- $timer));
                }
            } else {
                return $this->SetCode();
            }
        }else{
            if ($_SESSION['_'.$this->value]) {
                //拆分session
                $arr=explode('|',$_SESSION['_'.$this->value]);
                ...
                $timer=$arr[1];
                //当前时间  保存时间 过期了
                //$this->timer 有效期  60 秒是 2次短信发送的间隔
...
//           session($this->value,$this->code.'|'.time());
           $_SESSION['_'.$this->value]=$this->code.'|'.time();
//           die;
           return $arr=array('err'=>0,'invalid'=>$this->timer);
        }else{
            //4:短信发送失败
